...
public interface IEnergyNet{
	 ...
	 * 
	 * This is not the same as World.getTileEntity(), it's possible to register
	 * delegate tile entities with the energy net which are different from
	 * what's actually in the world. Those delegates allow to use separate
	 * TileEntity objects just for the EnergyNet interfaces, simplifying
	 * cross-mod dependencies and multi-blocks.
	 * 
	 * @param world
	 ...
	 * @param z
	 *            z-coordinate
	 * @return tile entity registered to the energy net or null if none is
	 *         registered
	 */
	TileEntity getTileEntity(World world, int x, int y, int z);

	/**
	 * Get the EnergyNet-registered neighbor tile entity at the specified
	 * position.
	 * 
	 * @param te
	 *            TileEntity indicating the world and position to search from
	 * @param dir
	 *            direction the neighbor is to be found
	 * @return neighbor tile entity registered to the energy net or null if none
	 *         is registered
	 */
	 ...
	 * 
	 * @note call this twice with x ticks delay to get the avg. emitted power p
	 *       = (call2 - call1) / x EU/tick
	 * 
	 * @param tileEntity
	 *            energy emitter
	 ...
	 * 
	 * @note call this twice with x ticks delay to get the avg. sunken power p =
	 *       (call2 - call1) / x EU/tick
	 * 
	 * @param tileEntity
	 *            energy emitter
	...
	/**
	 * Determine the typical power used by the specific tier, e.g. 128 eu/t for
	 * tier 2.
	 * 
	 * @param tier
	 *            tier
	...
	/**
	 * Determine minimum tier required to handle the specified power, e.g. tier
	 * 2 for 128 eu/t.
	 * 
	 * @param power
	 *            in eu/t
