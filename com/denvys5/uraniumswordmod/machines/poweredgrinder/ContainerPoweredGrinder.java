...
public class ContainerPoweredGrinder extends Container{
	...
	public void canCraftingToCrafters(ICrafting icrafting){
		...
		icrafting.sendProgressBarUpdate(this, 1, this.PoweredGrinder.storage.getEnergyStored());
	...
	public void detectAndSendChanges(){
		...
		for(int i = 0; i < this.crafters.size(); i++){
			...
			if(this.lastpower != this.PoweredGrinder.storage.getEnergyStored()){
				icrafting.sendProgressBarUpdate(this, 1, this.PoweredGrinder.storage.getEnergyStored());
			}
		}
		this.lastCookTime = this.PoweredGrinder.cookTime;
		this.lastpower = this.PoweredGrinder.storage.getEnergyStored();
	}

	@SideOnly(Side.CLIENT)
	public void updateProgressBar(int slot, int newValue){
		if(slot == 0) this.PoweredGrinder.cookTime = newValue;
		if(slot == 1) this.PoweredGrinder.storage.setEnergyStored(newValue);
	}
