...
public class TileEntityDuplicator extends TileEntity implements ISidedInventory {
	...
	public void updateEntity() {
		...
		boolean flag1 = false;
		if(operationSpeed() != 0){
			this.duplicatorSpeed = operationSpeed();
		}
		if (!this.worldObj.isRemote) {
			...
			if (flag != this.hasPower()) {
				Duplicator.updateDuplicatorBlockState(this.hasPower() && this.canGrind(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
		if (this.hasPower() && this.canGrind()) {
			this.cookTime++;
			this.power -= this.powerUsage;
			if (this.cookTime == this.duplicatorSpeed) {
				this.cookTime = 0;
				this.duplicateItem();
				flag1 = true;
			}
	...
	public int getGrinderProgressScaled(int i) {
		return this.cookTime * i / this.duplicatorSpeed;
