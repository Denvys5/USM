...
public class FurnaceUranium extends BlockContainer{
	...
	public Item getItemDropped(int par1, Random random, int par3){
		return Item.getItemFromBlock(USMTiles.furnaceuraniumidle);
	}
	...
	public static void updateFurnaceUraniumBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord){
		...
		keepInventory = true;
		if(active){
			worldObj.setBlock(xCoord, yCoord, zCoord, USMTiles.furnaceuraniumactive);
		} else{
			worldObj.setBlock(xCoord, yCoord, zCoord, USMTiles.furnaceuraniumidle);
		}
		keepInventory = false;
	...
	public void breakBlock(World world, int x, int y, int z, Block oldBlock, int oldMetadata){
		if(!keepInventory){
			TileEntityFurnaceUranium tileentity = (TileEntityFurnaceUranium)world.getTileEntity(x, y, z);
			if(tileentity != null){
